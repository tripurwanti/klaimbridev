...
FROM askred_file_rc_subrogasi fr");
...
while ($row = mssql_fetch_array($q)) {
    ...
    //     <a type='submit' class='btn btn-outline-success btn-sm' data-placement='bottom' 
    //     href='#'
    //     title='Posting' disabled><i class='fa fa-file'></i></a>
    // 	";
    // } else {
    $sub_array[] = "
        <a type='submit' href='media.php?module=detailDataRC&idfile=$row[id]'
		...
		&nbsp&nbsp&nbsp
        <a type='submit' class='btn btn-outline-success btn-sm' data-placement='bottom' 
        data-toggle='modal' data-target='#myModalPosting' data-whatever='modul/subrogasi/modalPostingSubrogasiView.php?fileId=" . $row['id'] . "' data-toggle='tooltip'
        title='Posting'><i class='fa fa-file'></i></a>
		";
    // }
    // $sub_array[] = "<a type='submit' href='modul/subrogasi/matchingProcess.php?id=" . $row['id'] . "' title='Reconcile'><i class='fa fa-file'></i></a></span>";
    // $sub_array[] = "<a type='submit' href='excel/export_excel_RC.php?file=" . $row['documen_name'] . "&id=" . $row['id'] . "' class='btn btn-outline-success btn-sm' data-toggle='tooltip' data-placement='bottom'
    // 	title='Download File Excel'><i class='fa fa-download'></i></a>
    // 	&nbsp&nbsp&nbsp

    // <a type='submit' class='btn btn-outline-success btn-sm' data-placement='bottom' 
    // data-toggle='modal' data-target='#myModal' data-whatever='modul/subrogasi/modalPostingSubrogasiView.php?fileId=" . $row['id'] . "&fileName=" . $row['documen_name'] . "' data-toggle='tooltip'
    // title='Posting'><i class='fa fa-file'></i></a>

    // 	";
    // <a type='submit' href='modul/subrogasi/matchingProcess.php?id=" . $row['id'] . "' title='Reconcile'><i class='fa fa-file'></i></a></span>
    // <a type='submit' href='modul/subrogasi/matchingProcess.php' title='Posting'><i class='fa fa-file'></i></a></span>
    // <a type='submit' class='btn btn-outline-success btn-sm' data-placement='bottom' 
    // data-toggle='modal' data-target='#myModal' data-whatever='modul/subrogasi/modalPostingSubrogasiView.php?fileId=" . $row['id'] . "&fileName=" . $row['documen_name'] . "' data-toggle='tooltip'
    // title='Posting'><i class='fa fa-file'></i></a>
    if ($row['status'] == '0') {
