            ...
            foreach ($Spreadsheet as $Key => $Row) {
                if ($FileType == "xlsx") {
                    if ($Key > 9) {
                        ...
                        // echo $remark . "break";
                        // var_dump(mssql_fetch_array($cekMatchDouble));

                        // data match
                        if (mssql_num_rows($cekMatch) > 0) {
                            // data match double
                            if (mssql_num_rows($cekMatchDouble) > 0) {
                                $ismatch = 2;
                                mssql_query("UPDATE asf
                                                ...
                                                AND asv.remark = '" . $remark . "'", $con);
                                while ($row = mssql_fetch_array($cekMatchDouble)) {
                                    mssql_query("UPDATE askred_mapping_rc_bri_claim set ismatch = '2' WHERE remark = '" . $remark . "' AND id = '" . $row['id'] . "'", $con);
                                }
                            } else {
                                $ismatch = 1;
                                mssql_query("UPDATE asf
