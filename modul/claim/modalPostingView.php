<form action="modul/claim/matchingProcess.php?file=<?php echo $_GET['fileName']; ?>&id=<?php echo $_GET['fileId']; ?>" method="post" id="form">
    ...
    <div class=col-sm-8 style="margin-bottom : 3px">
        ...
        <input type="text" class="form-control" name="no_mm" id="no_mm_<?php echo $_GET['fileId']; ?>" required>
    </div>
    <div class=col-sm-4 style="margin-bottom : 3px">
        <label for="example-search-input" class="col-form-label">Tanggal MM</label>
        <input type="date" class="form-control" name="tgl_mm" id="tgl_mm_<?php echo $_GET['fileId']; ?>" required>
    </div>
    <!-- <div class=col-sm-4 style="margin-bottom : 3px">
        <input type="text" class="form-control" name="filenama" value="<?php echo $_GET['fileName']; ?>">
        <input type="text" class="form-control" name="fileid" value="<?php echo $_GET['fileId']; ?>">
    </div> -->
    <br>
...
<script type="text/javascript" language="javascript">
    function posting(name, id) {
        ...
        if (no_bk == '' || tgl_bk == '' || no_mm == '' || tgl_mm == '') {
            ...
            $.ajax({
                method: "POST",
                url: "http://10.10.1.247:81/klaimbridev/modul/claim/matchingProcess.php?file=" + name + "&id=" + id,
                data: {
                    no_bk: no_bk,
                    no_mm: no_mm,
                    tgl_mm: tgl_mm,
                ...
                success: function(data) {
                    ...
                    $("#loading").hide();
                    alert("File berhasil di posting hehe");
                    window.location.href = 'http://10.10.1.247:81/klaimbridev/media.php?module=claimListRC';
                },
